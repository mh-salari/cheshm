...
#include "cheshm/glint/Simple/defaults.hpp"
...
bool passes_half_plane(const std::vector<cv::Point>& contour,
                       double cx,
                       double cy,
                       bool keep_above,
                       bool keep_below,
...
std::vector<Candidate> split_widest(std::vector<Candidate>&& accepted, const cv::Mat& glint_mask)
{
    if (accepted.empty())
    {
...
bool compute_center(const Candidate& cand, int method, double& cx, double& cy)
{
    switch (method)
    {
                                        ...
                                        double min_roundness_ratio)
{
    ...
    const bool half_plane_active = has_pupil && !(keep_above && keep_below && keep_left && keep_right);
    if (half_plane_active)
    {
        ...
        for (auto& c : contours)
        {
            if (passes_half_plane(
                    c, cx_local, cy_local, keep_above, keep_below, keep_left, keep_right, filter_margin_px))
            {
                kept.push_back(std::move(c));
            }
        }
    ...
    if (split_widest_for_target && glints_target > 1 && static_cast<int>(accepted.size()) == glints_target - 1 &&
        !accepted.empty())
    {
        accepted = split_widest(std::move(accepted), glint_mask);
    }

    std::sort(accepted.begin(),
              accepted.end(),
                  ...
                  double min_roundness_ratio)
{
    ...
    auto result = cheshm::SimpleGlint::detect_impl(full,
                                                   roi_x,
                                                   roi_y,
                                                   roi_w,
                                                   roi_h,
...
NB_MODULE(_core, m)
{
    ...
    m.def("detect",
          ...
          "pupil_radius"_a,
          "glint_threshold"_a,
          "search_radius_factor"_a,
          "search_radius_max_px"_a,
