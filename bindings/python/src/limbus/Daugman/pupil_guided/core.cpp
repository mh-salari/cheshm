...
#include "_common/nb_to_numpy.hpp"
                         ...
                         double k_max)
{
    ...
    auto result = cheshm::Daugman::pupil_guided::detect_limbus(
        full, {seed_x, seed_y}, pupil, n_angles, m_harmonics, gradient_sigma, radial_smoothing, k_min, k_max);
    if (!result)
        return nb::none();

    return nb::make_tuple(result->center.x,
                          result->center.y,
                          cheshm::Daugman::nb_to_numpy(std::move(result->thetas)),
                          cheshm::Daugman::nb_to_numpy(std::move(result->R_theta)));
}
