...
inline IntensityGapResult check_ellipse_intensity_gap(
    const cv::Mat &image,
    ...
    float threshold)
{
    ...
    if (inner_count <= 0.0f) return {false, 0.0f};
    ...
    for (int i = outer_box.x_start; i < outer_box.x_end; ++i) {
        for (int j = outer_box.y_start; j < outer_box.y_end; ++j) {
            const bool inside_cutout =
                i >= outer_cutout.x_start && i < outer_cutout.x_end
                && j >= outer_cutout.y_start && j < outer_cutout.y_end;
            if (inside_cutout) continue;
            if (i > 0 && i < image.cols && j > 0 && j < image.rows) {
    ...
    if (outer_count <= 0.0f) return {false, 0.0f};
