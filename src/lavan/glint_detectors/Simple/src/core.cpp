...
// entry point that lavan's Python wrapper loads via ctypes. Threshold
    ...
    double &cy)
{
    switch (method) {
        case CENTER_CONVEX_HULL_CENTROID: {
            ...
            const lavan::SplineCentroid sc = lavan::spline_polygon_centroid(hull, 200);
            if (sc.area == 0.0) {
                return false;
            }
            cx = sc.cx;
...
{
    using namespace lavan::SimpleGlint;  // NOLINT(google-build-using-namespace)
