#include "cheshm/limbus/Daugman/active_contour/radial_search.hpp"
...
// Bilinear sample of a float32 single-channel image. Returns
// nullopt when (x, y) is outside the safely-sampleable region.
                                  ...
                                  std::span<const double> smoothing_kernel)
{
    ...
    const float dr = (r_max - r_min) / static_cast<float>(n_r - 1);
    std::vector<float> radii(static_cast<std::size_t>(n_r));
    for (int j = 0; j < n_r; ++j)
        radii[j] = r_min + dr * static_cast<float>(j);
    ...
    {
        if (!eyelid_mask.empty() && eyelid_mask[i] != 0)
            continue;
        ...
        for (int j = 0; j < n_r; ++j)
        {
            const float x = seed_center.x + radii[j] * cos_t;
            const float y = seed_center.y + radii[j] * sin_t;
        ...
        if (best_idx >= 0 && std::isfinite(best_val))
            r_theta[i] = static_cast<double>(radii[best_idx]);
    }
