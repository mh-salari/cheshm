...
std::vector<cv::Point> Detector2D::ellipse_true_support(Detector2DProperties& props,
                                                        Ellipse& ellipse,
                                                        double ellipse_circumference,
                                                        std::vector<cv::Point>& raw_edges)
{
...
std::shared_ptr<Detector2DResult> Detector2D::detect(Detector2DProperties& props, cv::Mat& image, cv::Rect& roi)
{
    ...
    cv::inRange(pupil_image,
                cv::Scalar(0),
                cv::Scalar(lowest_spike_index + props.intensity_range),
                binary_img); // binary threshold
    kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, {7, 7});
    cv::dilate(binary_img, binary_img, kernel, {-1, -1}, 2);
    cv::inRange(
        pupil_image, cv::Scalar(0), cv::Scalar(highest_spike_index - spectral_offset), spec_mask); // binary threshold
    ...
    cv::Mat edges;
    cv::Canny(
        pupil_image, edges, props.canny_treshold, props.canny_treshold * props.canny_ration, props.canny_aperture);
    ...
    // split_contours = singleeyefitter::fun::filter( [](std::vector<cv::Point>& v){ return v.size() <= 3;} ,
    // split_contours);
    ...
    std::sort(split_contours.begin(),
              split_contours.end(),
              [](const Contour_2D& a, const Contour_2D& b) { return a.size() > b.size(); });
    const cv::Rect ellipse_center_varianz = cv::Rect(
        padding, padding, pupil_image.size().width - 2.0 * padding, pupil_image.size().height - 2.0 * padding);
    const EllipseEvaluation2D is_Ellipse(
        ellipse_center_varianz, props.ellipse_roundness_ratio, props.pupil_size_min, props.pupil_size_max);

    // finding potential candidates for ellipse seeds that describe the pupil.
    auto seed_contours = detector::divide_strong_and_weak_contours(split_contours,
                                                                   is_Ellipse,
                                                                   props.initial_ellipse_fit_treshhold,
                                                                   props.strong_perimeter_ratio_range_min,
                                                                   props.strong_perimeter_ratio_range_max,
                                                                   props.strong_area_ratio_range_min,
                                                                   props.strong_area_ratio_range_max);
    ...
    auto pruning_quick_combine = [&](const std::vector<std::vector<cv::Point>>& contours,
                                     ...
                                     int max_depth = 5)
    {
        ...
        int n = 0;
        std::generate(unknown.begin(),
                      unknown.end(),
                      [&]() { return Path{n++}; }); // fill with increasing values, starting from 0
        ...
        // even a superset is not tested again, because if a subset is bad, we can't make it better if more contours
        // are added
        ...
        while (!unknown.empty() && eval_count <= max_evals)
        {
            ...
            if (current_path.size() <= max_depth)
            {
                ...
                if (!includes_bad_paths)
                {
                    ...
                    for (int j : current_path)
                    {
                        size += contours.at(mapping.at(j)).size();
                    };
    ...
    Contours_2D split_contours_resolved(
        split_contours.size()); // WILL CONTAIN RESOLVED SPLIT CONTOURS TO TEST QUALITY OF CANDIDATE ELLIPSES
    ...
    double max_support_ratio =
        props.final_perimeter_ratio_range_min; // KEEPS TRACK OF MAXIMUM SUPPORT RATIO REACHED SO FAR
    ...
    for (auto& s : solutions)
    {
        ...
        for (int i : s)
        {
            ...
            if (split_contours_resolved[i].size() == 0)
            {
                ...
                split_contours_resolved[i].insert(
                    split_contours_resolved[i].end(), generated_edges.begin(), generated_edges.end());
            }
    ...
    auto final_fitting = [&](std::vector<std::vector<cv::Point>>& contours, cv::Mat& edges) -> std::vector<cv::Point>
    {
        ...
        // new_edges.at<int>(0,0) = 1; // find zero crashes if it doesn't find one. remove if opencv version is 3.0 or
        // above
        cv::findNonZero(new_edges, new_contours);
    ...
    std::vector<cv::Point> support_pixels =
        ellipse_true_support(props, result->ellipse, ellipse_circumference, final_edges);
    double support_ratio = support_pixels.size() / ellipse_circumference;
