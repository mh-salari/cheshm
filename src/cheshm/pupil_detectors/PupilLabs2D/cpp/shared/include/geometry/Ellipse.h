...
public:
    ...
    Scalar circumference() const
    {
        ...
        using std::sqrt;
        return constants::PI *
               abs(3.0 * (major_radius + minor_radius) -
                   sqrt(10.0 * major_radius * minor_radius + 3.0 * (pow(major_radius, 2) + pow(minor_radius, 2))));
    }
    Scalar area() const
    {
...
{
    return el1.center[0] == el2.center[0] && el1.center[1] == el2.center[1] && el1.major_radius == el2.major_radius &&
           el1.minor_radius == el2.minor_radius && el1.angle == el2.angle;
}
template <typename T, typename U>
bool operator!=(const Ellipse2D<T>& el1, const Ellipse2D<U>& el2)
{
...
{
    return os << "Ellipse { center: (" << ellipse.center[0] << "," << ellipse.center[1]
              << "), a: " << ellipse.major_radius << ", b: " << ellipse.minor_radius
              << ", theta: " << (ellipse.angle / constants::PI) << "pi }";
}
...
{
    return Ellipse2D<T>(
        ellipse.center[0].a, ellipse.center[1].a, ellipse.major_radius.a, ellipse.minor_radius.a, ellipse.angle.a);
}

template <class Scalar, class Scalar2>
inline Eigen::Matrix<typename std::common_type<Scalar, Scalar2>::type, 2, 1>
pointAlongEllipse(const Ellipse2D<Scalar>& el, Scalar2 t)
{
    using std::cos;
    using std::sin;
