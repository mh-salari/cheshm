...
{
    cvx::line(img,
              center + cv::Point2f(-radius, -radius),
              center + cv::Point2f(radius, radius),
              colour,
              thickness,
              lineType,
...
{
    cvx::line(
        img, center + cv::Point2f(0, -radius), center + cv::Point2f(0, radius), colour, thickness, lineType, shift);
    cvx::line(
        img, center + cv::Point2f(-radius, 0), center + cv::Point2f(radius, 0), colour, thickness, lineType, shift);
}
...
/*inline void cross(cv::Mat& img, cv::Point center, int radius, const cv::Scalar& colour, int thickness = 1, int
lineType = 8, int shift = 0)
{
    cv::line(img, center + cv::Point(-radius, -radius), center + cv::Point(radius, radius), colour, thickness,
lineType, shift); cv::line(img, center + cv::Point(-radius, radius), center + cv::Point(radius, -radius), colour,
thickness, lineType, shift);
}
inline void plus(cv::Mat& img, cv::Point center, int radius, const cv::Scalar& colour, int thickness = 1, int lineType
= 8, int shift = 0)
{
...
inline cv::Mat
GaussianBlur(const cv::Mat& src, cv::Size ksize, double sigmax, double sigmay = 0, int borderType = cv::BORDER_DEFAULT)
{
    cv::Mat dst;
...
inline cv::Mat
GaussianBlur(cv::Mat&& src, cv::Size ksize, double sigmax, double sigmay = 0, int borderType = cv::BORDER_DEFAULT)
{
