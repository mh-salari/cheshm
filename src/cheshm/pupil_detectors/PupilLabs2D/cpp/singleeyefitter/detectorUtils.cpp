...
void detector::calculate_spike_indices_and_max_intenesity(cv::Mat& histogram,
...
template <typename Scalar>
std::vector<int> detector::detail::find_kink_and_dir_change(const std::vector<Scalar>& curvature,
                                                            const Scalar max_angle)
{
...
template <typename Scalar>
Contours_2D detector::split_rough_contours_optimized(const Contours_2D& contours,
                                                     const Scalar max_angle,
                                                     const int min_contour_size)
{
    ...
    for (auto it = contours.begin(); it != contours.end(); it++)
    {
        ...
        for (auto point_it = contour.begin() + 2; point_it != contour.end(); point_it++)
        {
            ...
            {
                // we wanna split now
                current_contour_end_position =
                    (point_it - 1); // when we recognise a split, the middle point is the split point
                ...
                {
                    split_contours.emplace_back(last_contour_end_position,
                                                current_contour_end_position + 1); // range is [first, last)
                }
...
std::pair<ContourIndices, ContourIndices>
detector::divide_strong_and_weak_contours(const Contours_2D& contours,
                                          const EllipseEvaluation2D& is_ellipse,
                                          const float ellipse_fit_treshold,
                                          const float strong_perimeter_ratio_range_min,
                                          const float strong_perimeter_ratio_range_max,
                                          const float strong_area_ratio_range_min,
                                          const float strong_area_ratio_range_max)
{
    ...
    {
        if (contour.size() >= 5)
        { // because fitEllipse needs at least 5 points
            ...
            if (is_ellipse(ellipse))
            {
                ...
                auto sum_function = [&](cv::Point& point)
                { return std::pow(std::abs(ellipseDistance(point.x, point.y)), 2); };
...
std::pair<double, double> detector::ellipse_contour_support_ratio(const Ellipse& ellipse, const Contour_2D& contour)
{
    ...
    double perimeter_ratio =
        actual_length / ellipse.circumference(); // we assume here that the contour lies close to the ellipse boundary
...
// tell the compile to generate these explicit templates, otherwise it wouldn't know which one to create at compile
// time
...
template Contours_2D
detector::split_rough_contours_optimized(const Contours_2D& contours, const float angle, const int min_contour_size);
template Contours_2D
detector::split_rough_contours_optimized(const Contours_2D& contours, const double angle, const int min_contour_size);
