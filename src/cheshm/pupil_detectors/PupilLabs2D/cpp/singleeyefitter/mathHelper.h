...
#define MAKE_SQ(TYPE)                               \
    inline auto sq(TYPE val) -> decltype(val * val) \
    {                                               \
        return val * val;                           \
    }
...
T haversine(T theta1, T psi1, T theta2, T psi2)
{
    ...
    // Haversine distance
    auto dist = T(2) * asin(sqrt((sin((theta2 - theta1) / T(2)) * sin((theta2 - theta1) / T(2))) +
                                 cos(theta1) * cos(theta2) * (sin((psi2 - psi1) / T(2)) * sin((psi2 - psi1) / T(2)))));
    return dist;
}
