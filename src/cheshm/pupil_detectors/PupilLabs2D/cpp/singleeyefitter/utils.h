...
{
    return cv::Point(static_cast<int>(point[0]), static_cast<int>(point[1]));
}
template <typename Scalar>
inline cv::Mat toMat(const Eigen::Matrix<Scalar, 3, 1>& point)
{
    return (cv::Mat_<Scalar>(3, 1) << point[0], point[1], point[2]);
}
template <typename Scalar>
inline cv::Mat toMat(const Eigen::Matrix<Scalar, 2, 1>& point)
{
    return (cv::Mat_<Scalar>(2, 1) << point[0], point[1]);
}
template <typename Scalar>
inline cv::RotatedRect toRotatedRect(const Ellipse2D<Scalar>& ellipse)
{
    return cv::RotatedRect(
        toPoint2f(ellipse.center),
        cv::Size2f(static_cast<float>(2.0 * ellipse.major_radius), static_cast<float>(2.0 * ellipse.minor_radius)),
...
cv::Rect bounding_box(const Ellipse2D<Scalar>& ellipse)
{
    ...
    return cv::Rect(floor(ellipse.center[0] - bbox_halfwidth),
                    floor(ellipse.center[1] - bbox_halfheight),
                    2 * ceil(bbox_halfwidth) + 1,
                    2 * ceil(bbox_halfheight) + 1);
}
