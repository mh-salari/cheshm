// cv2 helpers used by Swirski 2D — small wrappers that take/return
// ``cv::Mat`` by value instead of via an output reference, plus a few
// ROI / drawing helpers. Header-only.
...
{
    cvx::line(img,
              centre + cv::Point2f(-radius, -radius),
              centre + cv::Point2f(radius, radius),
              colour,
              thickness,
              lineType,
...
{
    cvx::line(
        img, centre + cv::Point2f(0, -radius), centre + cv::Point2f(0, radius), colour, thickness, lineType, shift);
    cvx::line(
        img, centre + cv::Point2f(-radius, 0), centre + cv::Point2f(radius, 0), colour, thickness, lineType, shift);
}
