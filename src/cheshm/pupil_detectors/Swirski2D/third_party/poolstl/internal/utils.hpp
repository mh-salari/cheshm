...
template <typename Iterator>
constexpr typename std::iterator_traits<Iterator>::difference_type get_iter_chunk_size(
    const Iterator& iter, const Iterator& last, typename std::iterator_traits<Iterator>::difference_type chunk_size)
{
...
public:
    ...
    explicit getting_iter(Iterator iter)
        : iter(iter)
    {
    }
...
{
    pivot_predicate(Compare comp, const T& pivot)
        : comp(comp),
          pivot(pivot)
    {
    }
