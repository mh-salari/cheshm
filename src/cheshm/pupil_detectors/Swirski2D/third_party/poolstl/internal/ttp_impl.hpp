...
std::vector<std::future<void>> parallel_apply(ExecPolicy&& policy, Op op, const ArgContainer& args_list)
{
    ...
    for (const auto& args : args_list)
    {
        futures.emplace_back(
            task_pool.submit([](Op op, const auto& args_fwd) { std::apply(op, args_fwd); }, op, args));
    }
...
std::vector<std::future<ChunkRet>> parallel_chunk_for_gen(
    ExecPolicy&& policy,
    RandIt first,
    RandIt last,
    Chunk chunk,
...
std::vector<std::future<ChunkRet>> parallel_chunk_for_1(
...
parallel_chunk_for_1_wait(ExecPolicy&& policy,
                          ...
                          Chunk chunk,
                          ChunkRet* rettype,
                          int extra_split_factor,
                          A&&... chunk_args)
{
    auto futures = parallel_chunk_for_1(
        std::forward<ExecPolicy>(policy), first, last, chunk, rettype, extra_split_factor, chunk_args...);
    get_futures(futures);
}
...
std::vector<std::future<ChunkRet>> parallel_chunk_for_2(
    ExecPolicy&& policy, RandIt1 first1, RandIt1 last1, RandIt2 first2, Chunk chunk, ChunkRet*, A&&... chunk_args)
{
    std::vector<std::future<ChunkRet>> futures;
    auto& task_pool = *policy.pool();
 ...
 */
template <class ExecPolicy, class RandIt1, class RandIt2, class RandIt3, class Chunk, class ChunkRet, typename... A>
std::vector<std::future<ChunkRet>> parallel_chunk_for_3(ExecPolicy&& policy,
                                                        RandIt1 first1,
                                                        RandIt1 last1,
                                                        RandIt2 first2,
                                                        RandIt3 first3,
...
void parallel_mergesort(
    ExecPolicy&& policy, RandIt first, RandIt last, Compare comp, SortFunc sort_func, MergeFunc merge_func)
{
    if (first == last)
    {
        return;
    ...
    auto futures = parallel_chunk_for_gen(std::forward<ExecPolicy>(policy),
                                          first,
                                          last,
        ...
        for (std::size_t i = 0; i < subranges.size(); ++i)
        {
            if (i + 1 < subranges.size())
            {
                ...
                futures.emplace_back(task_pool.submit(
                    [&comp, merge_func](RandIt chunk_first, RandIt chunk_middle, RandIt chunk_last)
                    {
...
void quicksort_impl(task_thread_pool::task_thread_pool* task_pool,
                    const RandIt first,
                    const RandIt last,
                    Compare comp,
                    SortFunc sort_func,
                    ...
                    int* inflight_spawns)
{
    ...
    if (partition_size > target_leaf_size)
    {
        ...
        if (mid != first && mid != last)
        {
            ...
            futures->emplace_back(task_pool->submit(quicksort_impl<RandIt, Compare, SortFunc, PartFunc, PivotFunc>,
                                                    task_pool,
                                                    first,
                                                    mid,
                                                    comp,
                        ...
                        PivotFunc pivot_func)
{
    ...
    std::ptrdiff_t target_leaf_size =
        std::max((std::ptrdiff_t)(std::distance(first, last) / (num_threads * 2)), (std::ptrdiff_t)5);
