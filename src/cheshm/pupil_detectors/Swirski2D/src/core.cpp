...
nb::object detect(
    ...
    double canny_blur, double canny_thresh1, double canny_thresh2,
    ...
    int max_inliers)
{
    ...
    nb::ndarray<nb::numpy, double, nb::ndim<2>> inliers_arr(
        inliers_data, 2, inliers_shape, inliers_cap);

    return nb::make_tuple(
        nb::make_tuple(cx, cy, w, h, angle),
        std::move(inliers_arr));
}
...
NB_MODULE(_core, m)
{
    ...
    m.def("detect", &detect,
          ...
          "canny_blur"_a, "canny_thresh1"_a, "canny_thresh2"_a,
          "starburst_points"_a,
          "percentage_inliers"_a, "inlier_iterations"_a,
