...
{
public:
    EllipseEvaluation2D(const cv::Rect& centerVarianz,
                        const float roundness_ratio,
                        const float size_min,
                        const float size_max)
        : centerVarianz(centerVarianz),
    ...
    {
        bool is_centered =
            centerVarianz.x < ellipse.center.x && ellipse.center.x < (centerVarianz.width + centerVarianz.x) &&
