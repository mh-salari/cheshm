...
// returns lowest and highest intensity index and max intensity. only intensity values wich occure more than
// amount_intensity_values are taken in to account
...
// splits contours in independent contours, satisfying that all single contours have the same curvature, at least
// min_contour_size points, and the maximum angle between three points is max_angle
template <typename Scalar>
Contours_2D
split_rough_contours_optimized(const Contours_2D& contours, const Scalar max_angle, const int min_contour_size);

// returns the indices to strong and weak contours
std::pair<ContourIndices, ContourIndices> divide_strong_and_weak_contours(const Contours_2D& contours,
