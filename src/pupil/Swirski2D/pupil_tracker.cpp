...
#include "cheshm/pupil/Swirski2D/pupil_tracker.hpp"

#include "cheshm/pupil/Swirski2D/cvx.hpp"
...
cv::RotatedRect fitEllipse(const std::vector<EdgePoint>& edgePoints)
{
    std::vector<cv::Point2f> points;
    points.reserve(edgePoints.size());

    for (const EdgePoint& e : edgePoints)
...
bool findPupilEllipse(const TrackerParams& params, const cv::Mat& m, findPupilEllipse_out& out)
{
    ...
    std::vector<cv::Point2f> inliers;
    {
        ...
        out.pPupil = pPupil;
        out.elPupil = elPupil;
        // Translate RANSAC inliers from ROI-local back into the image
