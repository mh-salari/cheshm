...
#include "cheshm/pupil/Starburst/corneal_reflection.hpp"
...
void locate_corneal_reflection(
    cv::Mat& image, int sx, int sy, int window_size, int biggest_crar, int& crx, int& cry, int& crar)
{
    if (window_size % 2 == 0)
    {
...
int fit_circle_radius_to_corneal_reflection(cv::Mat& image,
                                            int crx,
                                            int cry,
                                            int crar,
                                            int biggest_crar,
...
void interpolate_corneal_reflection(
    cv::Mat& image, int crx, int cry, int crr, const double* sin_array, const double* cos_array, int array_len)
{
    ...
    for (int r = 1; r < crr; r++)
    {
        ...
        for (int i = 0; i < array_len; i++)
        {
            ...
            *(image.data + y * image.size().width + x) =
                static_cast<std::uint8_t>((r2 * 1.0 / crr) * avg + (r * 1.0 / crr) * perimeter_pixel[i]);
        }
    }
}
    ...
    cv::Mat& image, int sx, int sy, int window_size, int biggest_crr, int& crx, int& cry, int& crr)
{
    ...
    crr = fit_circle_radius_to_corneal_reflection(
        image, crx, cry, crar, static_cast<int>(biggest_crr / 2.5), sin_array.data(), cos_array.data(), angle_num);
