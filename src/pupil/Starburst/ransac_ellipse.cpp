...
#include "cheshm/pupil/Starburst/ransac_ellipse.hpp"
...
int* RansacEllipse::pupil_fitting_inliers(std::uint8_t* /*pupil_image*/,
                                          int width,
                                          int height,
    ...
    while (sample_num > ransac_count)
    {
        ...
        for (int i = 0; i < ep_num; i++)
        {
            dis_error = conic_par[0] * edge_point_nor[i].x * edge_point_nor[i].x +
                        ...
                        conic_par[2] * edge_point_nor[i].y * edge_point_nor[i].y + conic_par[3] * edge_point_nor[i].x +
        ...
        if (ninliers > max_inliers)
        {
            if (solve_ellipse(conic_par, ellipse_par))
            {
                ...
                if (ellipse_par[2] > 0 && ellipse_par[2] <= width - 1 && ellipse_par[3] > 0 &&
                    ellipse_par[3] <= height - 1 && ratio > 0.5 && ratio < 2)
                {
                    ...
                    sample_num =
                        static_cast<int>(std::log(1.0 - 0.99) / std::log(1.0 - std::pow(ninliers * 1.0 / ep_num, 5)));
                }
            }
        }
        ransac_count++;
